...
大道至简，无需寻找其他利用文件，覆写文件本身。

群主杀死了比赛，给这个方案划上了一个圆满的句号，如果是 sudo 文件，能控制写入其中一行内容，那么直接覆盖整个文件。如果不是二进制文件或没有使用 `#!` 指定解释器，默认作为 shell 脚本解析，bash 脚本默认遇到错误行仍然会继续往下走。
...
![](/medias/linux-config-privesc/25.png)


成功控制文件写入内容，覆写 dirsearch，现在sudo 执行dirsearch 就会被当成 bash 脚本来执行

![](/medias/linux-config-privesc/26.png)
