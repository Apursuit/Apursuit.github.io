...
SQLmap官方文档[SQLmap](https://github.com/sqlmapproject/sqlmap/wiki/usage)


本来打算写一个sqlmap配套靶场放在github上水一下，在设计靶场时，发现太难了，作为入门靶场好像太过于文档化教学了，不如水一个博客了。

## sqlmap安装

sqlmap 是用 Python 编写的，运行需要python环境，[下载安装python](https://www.python.org/downloads/)

kali自带了sqlmap，直接在终端输入`sqlmap`即可使用。

windows用户可以直接下载[sqlmap](https://github.com/sqlmapproject/sqlmap/archive/refs/tags/1.9.zip)，解压后在终端进入sqlmap目录，输入`python sqlmap.py`即可使用。
