...
确定需求，在Dockerhub中搜索PHP，官方给出很多镜像，在标签页找php-apache相关镜像  
...
在主机/虚拟机中准备src目录，使用你的编辑器/vi写入题目，配置文件(**没有特殊需求，不用准备配置文件，使用镜像中默认的**) 
...
现在你可以将主机/虚拟机src下的ini配置文件删掉了，后面index.php，flag.php还会用

## 6.正确配置php.ini文件

上一步将PHP配置文件复制到了/tmp目录下，并不会生效，我们要配置到正确的目录下
...
现在你已经把配置文件放到正确的目录下了，看起来可以达到我的预期了(滑稽)
